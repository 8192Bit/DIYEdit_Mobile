...
// they said this file has been changed but seems nothing happen. why
// ln 200, 201
/*
...
public class DSSave implements Save {
    ...
    public DSSave(byte[] read1) {
        ...
        while (pos < 4) {
            ver1 += (saveFile[20 + pos] & 0xFF) << (8 * pos);
        ...
        pos = 0;
        while (pos < 4) {
            ver2 += (saveFile[12308 + pos] & 0xFF) << (8 * pos);
    ...
    private void setMio(int slotNumber, int mode, byte[] mio, boolean delete) {
        ...
        if (delete) {
            ...
            } else if (mio.length == data_size && free_offsets.size() > 0) {
